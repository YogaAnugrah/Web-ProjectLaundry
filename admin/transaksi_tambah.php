...
$paket = ambildata($conn,'SELECT id_paket, nama_paket, outlet.nama_outlet FROM paket INNER JOIN outlet 
ON paket.outlet_id = outlet.id_outlet;');
...
if(isset($_POST['btn-simpan'])){   
  ...
  $query = "INSERT INTO transaksi (outlet_id,kode_invoice,member_id,tgl,batas_waktu,biaya_tambahan,diskon,pajak,
  status,status_bayar,user_id) VALUES 
  ('$outlet_id',
  '$kode_invoice',
  '$member_id',
  '$tgl_sekarang',
  ...
  if ($execute == 1) {
    ...
    $sqlDetail = "INSERT INTO detail_transaksi (transaksi_id,paket_id,qty,total_harga) VALUES 
    ('$transaksi_id','$paket_id','$qty','$total_harga')";
  ...
  <div class="panel panel-container">
    <div class="row" style="padding: 0 15px 20px 15px;">
      <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
        <div class="col-md-6">
          <a href="transaksi_cari_member.php" class="btn btn-primary box-title">
        ...
        <div class="col-md-6 text-right">
          <button id="btn-refresh" class="btn btn-primary box-title text-right" title="Refresh Data">
            <i class="fa fa-refresh" id="ic-refresh"></i></button>
        </div>
      </div>
    </div>
  </div>
        ...
        <div style="padding: 0 30px 30px 30px;">
          <form method="post" action="">
            ...
            <div class="form-group">
              ...
              <input type="text" name="pelanggan" class="form-control" readonly="" value="
              <?= $member['nama_member'] ?>"> 
            </div>
            <div class="form-group">
              ...
              <?php foreach ($paket as $key): ?>
                <option value="<?= $key['id_paket'] ?>"><?= $key['nama_paket']." - ".$key['nama_outlet']  ?>
              </option>
              <?php endforeach ?>
              </select>
            </div>
