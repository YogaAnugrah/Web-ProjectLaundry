...
$title = 'Outlet';
require'admin.php';
$query = 'SELECT outlet.*, user.nama_user,user.id_user FROM outlet LEFT JOIN user 
          ON user.outlet_id = outlet.id_outlet WHERE id_outlet = ' . stripslashes($_GET['id']);
$data = ambilsatubaris($conn,$query);
$query2 = 'SELECT user.*,outlet.nama_outlet FROM outlet RIGHT JOIN user ON user.outlet_id = outlet.id_outlet 
          WHERE user.role = "Owner" order by user.outlet_id asc';
...
if(isset($_POST['btn-simpan'])){
  ...
  telp_outlet='$telp' 
  WHERE id_outlet = " . stripslashes($_GET['id']);
  
  if($_POST['owner_id_new']){
    $query2 = "UPDATE user SET outlet_id = '" . stripslashes($_GET['id']) . "' WHERE id_user = " .
    $_POST['owner_id_new'];
    $query3 = "UPDATE user SET outlet_id = NULL WHERE id_user = " . stripslashes($data['id_user']);
    $execute3 = bisa($conn,$query3);
  } else {
    $query2 = "UPDATE user SET outlet_id = '" . stripslashes($_GET['id']) . "' WHERE id_user = " . 
    stripslashes($_POST['owner_id']);
  }
        ...
        <div style="padding: 0 30px 30px 30px;">
          <form method="post" action="">
            ...
            <div class="form-group">
              ...
              <textarea name="alamat_outlet" class="form-control"><?= htmlspecialchars($data['alamat_outlet']); ?>
            </textarea>
            </div>
            <div class="form-group">
              <label>Nomor Telepon</label>
              <input type="text" value="<?= htmlspecialchars($data['telp_outlet']); 
              ?>" name="telp_outlet" class="form-control">
            </div>
