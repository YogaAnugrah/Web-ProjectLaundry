...
$title = 'Transaksi';
require'admin.php';
$query = 'SELECT transaksi.*,member.nama_member , detail_transaksi.total_harga FROM transaksi INNER JOIN member 
          ON member.id_member = transaksi.member_id INNER JOIN detail_transaksi 
          ON detail_transaksi.transaksi_id = transaksi.id_transaksi WHERE transaksi.id_transaksi = ' . $_GET['id'];
$data = ambilsatubaris($conn,$query);

if(isset($_POST['btn-simpan'])) {
    $total_bayar = $_POST['total_bayar'];
    if($total_bayar >= $data['total_harga']){
        $query = "UPDATE transaksi SET status ='Selesai', status_bayar = 'dibayar',tgl_pembayaran = '" . 
                  Date('Y-m-d h:i:s') . "' WHERE id_transaksi = " . $_GET['id'];
  ...
  <div class="panel panel-container">
    <div class="row" style="padding: 0 15px 20px 15px;">
      <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
        <div class="col-md-6">
          <a href="transaksi_konfirmasi.php" class="btn btn-primary box-title">
        ...
        <div style="padding: 0 30px 30px 30px;">
          <form method="post" action="transaksi_bayar.php?id=<?= $data['id_transaksi'] ?>" id="form-submit">
            <div class="form-group">
              ...
              <input type="text" name="kode_invoice" value="<?= $data['kode_invoice'] 
              ?>" class="form-control" readonly>
            </div>

            <div class="form-group">
              <label>Nama Member</label>
              <input type="text" name="nama_member" value="<?= $data['nama_member'] 
              ?>" class="form-control" readonly>
            </div>

            <div class="form-group">
              <label>Total Yang Harus Di Bayar</label>
              <input type="text" name="total_harga" value="<?= $data['total_harga'] 
              ?>" class="form-control" readonly>
            </div>
