...
WHERE status_bayar = 'Dibayar' AND WEEK(tgl_pembayaran) = WEEK(NOW()) AND transaksi.outlet_id = $outlet_id");

$bulan = ambilsatubaris($conn,"SELECT SUM(total_harga) AS total FROM detail_transaksi 
INNER JOIN transaksi ON transaksi.id_transaksi = detail_transaksi.transaksi_id 
WHERE status_bayar = 'Dibayar' AND MONTH(tgl_pembayaran) = MONTH(NOW()) AND transaksi.outlet_id = $outlet_id");

$tahun = ambilsatubaris($conn,"SELECT SUM(total_harga) AS total FROM detail_transaksi 
INNER JOIN transaksi ON transaksi.id_transaksi = detail_transaksi.transaksi_id 
WHERE status_bayar = 'Dibayar' AND YEAR(tgl_pembayaran) = YEAR(NOW()) AND transaksi.outlet_id = $outlet_id");

$penjualan = ambildata($conn,"SELECT SUM(detail_transaksi.total_harga) AS total,COUNT(detail_transaksi.paket_id) 
as jumlah_paket,paket.nama_paket,transaksi.tgl_pembayaran FROM detail_transaksi
INNER JOIN transaksi ON transaksi.id_transaksi = detail_transaksi.transaksi_id
INNER JOIN paket ON paket.id_paket = detail_transaksi.paket_id
WHERE transaksi.status_bayar = 'Dibayar' AND transaksi.outlet_id = $outlet_id 
GROUP BY detail_transaksi.paket_id");
    ...
    <div class="row">
      <div class="col-xs-6 col-md-4 col-lg-4 no-padding">
        <div class="panel panel-teal panel-widget border-right">
          <div class="row no-padding"><i class="fa fa-xl fa-hand-holding-usd color-orange"></i>
            <div class="large"><?= htmlspecialchars($minggu['total'] == null ? 0 : 'Rp. '.
            $minggu['total']); ?></div>
      ...
      <div class="col-xs-6 col-md-4 col-lg-4 no-padding">
        <div class="panel panel-blue panel-widget border-right">
          <div class="row no-padding"><i class="fa fa-xl fa-hand-holding-usd color-blue"></i>
            <div class="large"><?= htmlspecialchars($bulan['total'] == null ? 0 : 'Rp. '.
            $bulan['total']); ?></div>
      ...
      <div class="col-xs-6 col-md-4 col-lg-4 no-padding">
        <div class="panel panel-orange panel-widget border-right">
          <div class="row no-padding"><i class="fa fa-xl fa-hand-holding-usd color-red"></i>
            <div class="large"><?= htmlspecialchars($tahun['total'] == null ? 0 : 'Rp. '.
            $tahun['total']); ?></div>
    ...
    <div class="col-md-12 col-lg-12 col-sm-12">
      <div class="panel panel-container">
        <div style="padding: 0 30px 30px 30px;">
          <h3 style="padding: 0 0 20px 0;">Laporan Penjualan Paket <strong><?= 
          $nama_outlet['nama_outlet']; ?></strong></h3>
